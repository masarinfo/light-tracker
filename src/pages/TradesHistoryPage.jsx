  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="glass-panel p-4 rounded-2xl border border-white/10 flex flex-wrap items-center gap-4 text-xs">
        ...
        <select value={selectedCoin} onChange={(e) => setSelectedCoin(e.target.value)} className="p-2.5 rounded-xl glass-input text-white font-semibold min-w-[130px]">
        ...
        <select value={selectedExchange} onChange={(e) => setSelectedExchange(e.target.value)} className="p-2.5 rounded-xl glass-input text-white font-semibold min-w-[140px]">
          <option value="ALL" className="bg-gray-900">الكل</option>
          {exchangesList.map((exName) => (<option key={exName} value={exName} className="bg-gray-900">{exName}</option>))}
        </select>

        <select value={selectedStrategy} onChange={(e) => setSelectedStrategy(e.target.value)} className="p-2.5 rounded-xl glass-input text-white font-semibold min-w-[150px]">
          <option value="ALL" className="bg-gray-900">الكل</option>
          {strategiesList.map((stName) => (<option key={stName} value={stName} className="bg-gray-900">{stName}</option>))}
        </select>

        <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="p-2.5 rounded-xl glass-input text-white font-bold min-w-[160px]">
          <option value="ALL" className="bg-gray-900">الكل</option>
          ...
          <table className="w-full text-left text-xs font-sans">
            <thead className="bg-white/5 text-gray-400 font-semibold border-b border-white/10 uppercase tracking-wider">
              <tr>
                ...
                <th className="p-4">الاستراتيجية</th>
                <th className="p-4">الحالة</th>
                <th className="p-4">سعر الدخول</th>
                <th className="p-4">المبلغ المستثمر</th>
            ...
            <tbody className="divide-y divide-white/5 text-gray-200 font-mono">
              {filteredTrades.length === 0 ? (
                <tr><td colSpan="9" className="p-6 text-center text-gray-500 font-sans">لا يوجد صفقات مسجلة.</td></tr>
              ) : (

...
export default function Sidebar() {
  ...
  if (user?.is_superadmin) {
    mainNavItems.push({ id: 'users-management', label: lang === 'ar' ? 'إدارة المستخدمين' : 'Users Mgt', icon: Users, highlight: true });
    mainNavItems.push({ id: 'coupons-management', label: lang === 'ar' ? 'إدارة الكوبونات' : 'Coupons', icon: Tag, highlight: true });
    mainNavItems.push({ id: 'system-logs', label: lang === 'ar' ? 'سجل النظام (Admin)' : 'System Logs', icon: ShieldAlert, highlight: true });
  }
  ...
  return (
    <aside
      className={`glass-panel flex flex-col justify-between h-screen sticky top-0 z-30 shrink-0 transition-all duration-300 ease-in-out ${
        isSidebarCollapsed ? 'w-20' : 'w-72'
      } ${isRtl ? 'border-l border-white/10' : 'border-r border-white/10'}`}
    >
      {/* Top Header & Brand */}
      <div>
        <div className={`p-4 border-b border-white/10 flex flex-col gap-2 ${isSidebarCollapsed ? 'items-center' : ''}`}>
        ...
        <nav className="p-3 space-y-1 overflow-y-auto max-h-[calc(100vh-220px)]">
          ...
          {!isSidebarCollapsed && (
            <div className="pt-4 px-3 py-1 text-[11px] font-bold text-gray-400 uppercase tracking-wider flex items-center justify-between">
      ...
      <div className="p-3 border-t border-white/10">
        <button
          onClick={() => setActiveScreen('security-preview')}
          title={isSidebarCollapsed ? t('navSecurityPreview') : undefined}
          className={`w-full flex items-center ${
            isSidebarCollapsed ? 'justify-center p-3' : 'justify-between p-2.5'
          } rounded-xl border text-xs font-semibold transition-all ${
            activeScreen === 'security-preview'
        ...
        >
          <div className={`flex items-center gap-2.5 min-w-0 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            ...
            {!isSidebarCollapsed && <span className="truncate">{t('navSecurityPreview')}</span>}
          </div>
          {!isSidebarCollapsed && (
